# ==============================================================================
# 🏛️ HONG KONG ELECTORAL INTEL ENGINE: 20-YEAR HISTORICAL DATA MATRIX (v19.0)
# ==============================================================================
...
# 1. 網頁全螢幕與架構配置
st.set_page_config(layout="wide", page_title="香港二十年選戰大數據中心 v19.0", page_icon="🇭🇰")
st.title("🏛️ 香港地方選舉量化模擬與戰略推演系統 (歷史矩陣完全體 v19.0)")
st.subheader("📊 整合 2000-2019 區議會與立法會歷屆真實數據：黃藍大盤動態推射系統")
st.markdown("---")

# 2. 核心大數據資料庫（橫跨 2000-2019 歷屆真實選舉數據平滑化後之 18 區終極大盤）
# 支持率權重已將 2019 海嘯（黃營天花板）與 2000-2015 樁腳政治（藍營基本盤）完全融合精算
raw_historical_matrix = {
    "District": ["中西區", "灣仔區", "東區", "南區", "油尖旺區", "深水埗區", "九龍城區", "黃大仙區", "觀塘區", "葵青區", "荃灣區", "屯門區", "元朗區", "北區", "大埔區", "沙田區", "西貢區", "離島區"],
    "Population":,
    # 基於 20 年真實數據：市區、中產、科技城黃大於藍；公屋、北角閩籍、新界圍村藍大於黃
...
# 3. 側邊控制面板：完全對齊 2000-2019 歷史大選特性之四大核彈級政治滑桿
...
# 4. 歷史矩陣聯動演算核心公式 (The Electoral Matrix Algorithm)
...
# 定義香港特殊選區地緣政治遮罩 (Geopolitical Masks)
...
with stat_col3:
    ...
    fig_zones.add_trace(go.Bar(name='建制派', x=urban_df['District'], y=urban_df['Hist_Est_Avg']*100 if 'Hist_Est_Avg' in urban_df.columns else urban_df['Final_Est']*100, marker_color=party_pro_colors['Establishment']))
    fig_zones.update_layout(template="plotly_dark", title="都會核心（港島/九龍）二十年拉鋸盤勢 (%)", height=220, margin=dict(l=10, r=10, t=40, b=10), barmode='group', showlegend=False)
